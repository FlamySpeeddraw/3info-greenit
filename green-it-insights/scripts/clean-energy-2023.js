...
const [inputArg, outputArg, yearArg] = process.argv.slice(2);
const inputPath = inputArg || path.join(__dirname, '../public/data/energy-use-per-country/OWID_CB_ENERGY_PER_CAPITA.csv');
const year = yearArg || '2021';
const outputPath = outputArg || path.join(__dirname, `../public/data/energy-use-per-country/OWID_CB_ENERGY_PER_CAPITA.${year}.clean.csv`);
...
if (filtered.length === 0) {
  console.warn(`Aucune ligne trouvée pour l'année ${year}. Vérifiez que le fichier contient bien des données pour cette année.`);
}
